...
{
    public class PlaylistDetailPage : UserControl
    {
        ...
        public void LoadData(string playlistId, string name)
        {
            _lblName.Text = name;
            _list.Items.Clear();
            ...
            worker.DoWork += (s, e) =>
            {
                e.Result = App.Client.GetPlaylistTracks((string)e.Argument);
            };
            worker.RunWorkerCompleted += (s, e) =>
            {
                ...
                FillList((List<TrackItem>)e.Result);
            };
            worker.RunWorkerAsync(playlistId);
        }

        public void LoadAlbumData(string albumId, string name)
        {
            _lblName.Text = name;
            _list.Items.Clear();
            if (Controls.Contains(_lblError)) Controls.Remove(_lblError);
            ...
            worker.DoWork += (s, e) =>
            {
                e.Result = App.Client.GetAlbumSongs((string)e.Argument);
            };
            worker.RunWorkerCompleted += (s, e) =>
            {
                if (e.Error != null)
            ...
            worker.RunWorkerAsync(albumId);
        }

        private void FillList(List<TrackItem> tracks)
        {
            _list.Items.Clear();
            int num = 1;
